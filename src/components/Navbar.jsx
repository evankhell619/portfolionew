    ...
    <nav className={`fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50 pointer-events-none transition-all duration-500`}>
      ...
      <GsapPresence>
        {isMenuOpen && (
          ...
          >
            ...
            <div className="absolute -top-24 -right-16 w-72 h-72 rounded-full bg-pink-400/15 blur-3xl pointer-events-none" />
      ...
      <div className="hidden lg:flex pointer-events-auto">
        <Magnetic>
          <button
            onClick={() => scrollTo('contact-section')}
            className={`group relative overflow-hidden flex items-center gap-3 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-[0.12em] md:tracking-[0.16em] hover:shadow-[0_0_30px_rgba(244,114,182,0.3)] transition-all duration-500 ${isOnDarkSection ? 'bg-white text-black' : 'bg-black text-white'}`}
          >
            {/* Core Label */}
            <span className="relative z-10 pl-2">Let's Talk</span>

            {/* Glowing Icon Container */}
            <div className="relative z-10 w-7 h-7 bg-pink-400 rounded-full flex items-center justify-center text-black group-hover:scale-110 transition-transform duration-300">
