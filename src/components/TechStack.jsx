...
import { Workflow, Globe, Cloud } from "lucide-react";
                        ...
                        {stackCategories.map((category, index) => (
                            ...
                            >
                                <div className="md:w-1/3 shrink-0 flex flex-col gap-2">
                                    <h4 className="text-xl md:text-2xl font-bold uppercase text-white tracking-tight group-hover:text-pink-400 transition-colors">
                                        {category.title}
                                    </h4>
                                ...
                                <div className="md:w-2/3 flex flex-wrap gap-4 lg:gap-5 items-center">
                                    {category.skills.map((skill, idx) => {
                                        ...
                                        return (
                                            <div
                                                key={idx}
                                                className="relative group/icon w-11 h-11 md:w-12 md:h-12 flex items-center justify-center border border-white/10 rounded-lg hover:border-pink-400/50 hover:bg-pink-400/10 transition-all duration-300 cursor-default"
                                                title={skill.name}
                                            >
                                                <IconComponent className="w-5 h-5 md:w-6 md:h-6 text-white/60 group-hover:text-white/80 group-hover/icon:text-pink-400 transition-colors" />
                                                {/* Tooltip */}
